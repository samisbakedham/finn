...
use crate::finn::sync::body_sync::BodySync;
use crate::finn::sync::header_sync::HeaderSync;
use crate::finn::sync::state_sync::StateSync;
use crate::p2p;
...
impl SyncRunner {
	...
	fn wait_for_min_peers(&self) -> Result<(), chain::Error> {
		...
		loop {
			...
			// * we are synced already, e.g. finn was quickly restarted
			// * timeout
			if wp > MIN_PEERS
				|| (wp == 0
