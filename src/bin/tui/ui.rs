...
impl UI {
	...
	pub fn new(
		...
		//set theme
		let mut theme = finn_ui.cursive.current_theme().clone();
		modify_theme(&mut theme);
		finn_ui.cursive.set_theme(theme);
		finn_ui.cursive.add_fullscreen_layer(main_layer);

		// Configure a callback (shutdown, for the first test)
		let controller_tx_clone = finn_ui.controller_tx.clone();
		finn_ui.cursive.add_global_callback('q', move |c| {
