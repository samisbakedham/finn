...
fn get_finn_path(chain_type: &global::ChainTypes) -> Result<PathBuf, ConfigError> {
	// Check if finn dir exists
	let mut finn_path = match dirs::home_dir() {
		Some(p) => p,
		None => PathBuf::new(),
	};
	finn_path.push(finn_HOME);
	finn_path.push(chain_type.shortname());
	// Create if the default path doesn't exist
	if !finn_path.exists() {
		fs::create_dir_all(finn_path.clone())?;
	}
	Ok(finn_path)
}
...
fn check_api_secret_files(
	...
	let finn_path = get_finn_path(chain_type)?;
	let mut api_secret_path = finn_path;
...
pub fn initial_setup_server(chain_type: &global::ChainTypes) -> Result<GlobalConfig, ConfigError> {
	...
	// Use config file if current directory if it exists, .finn home otherwise
	if let Some(p) = check_config_current_dir(SERVER_CONFIG_FILE_NAME) {
		...
		let mut config_path = finn_path.clone();
		...
		if !config_path.exists() {
			...
			default_config.update_paths(&finn_path);
...
impl GlobalConfig {
	...
	/// Update paths
	pub fn update_paths(&mut self, finn_home: &PathBuf) {
		// need to update server chain path
		let mut chain_path = finn_home.clone();
		chain_path.push(finn_CHAIN_DIR);
		self.members.as_mut().unwrap().server.db_root = chain_path.to_str().unwrap().to_owned();
		let mut secret_path = finn_home.clone();
		...
		let mut log_path = finn_home.clone();
