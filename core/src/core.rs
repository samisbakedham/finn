...
pub fn amount_from_hr_string(amount: &str) -> Result<u64, Error> {
	...
	let (finns, nfinns) = match amount.find('.') {
		None => (parse_finns(amount)?, 0),
		Some(pos) => {
			let (gs, tail) = amount.split_at(pos);
			(parse_finns(gs)?, parse_nfinns(&tail[1..])?)
		}
	};
	Ok(finns * finn_BASE + nfinns)
}

fn parse_finns(amount: &str) -> Result<u64, Error> {
	if amount == "" {
		Ok(0)
	} else {
		amount
...
lazy_static! {
	static ref WIDTH: usize = (finn_BASE as f64).log(10.0) as usize + 1;
}

fn parse_nfinns(amount: &str) -> Result<u64, Error> {
...
pub fn amount_to_hr_string(amount: u64, truncate: bool) -> String {
	let amount = (amount as f64 / finn_BASE as f64) as f64;
