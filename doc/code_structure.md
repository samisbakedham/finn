...
finn is built in [Rust](https://www.rust-lang.org/), a memory safe, compiled language. Performance critical parts like the Cuckoo mining algorithm are built as plugins, making it easy to swap between algorithm implementations for various hardware. finn comes with CPU and experimental GPU support.
...
- [CONTRIBUTING](../CONTRIBUTING.md) - How to help and become part of finn.
...
After checking out finn, building and using, these are the folders you'll have:
...
- `doc`\
 All documentation.
- `servers`\
 Many parts (adapters, lib, miner, seed, server, sync, types) that the `finn` server needs, including mining server.
- `keychain`\
...
- `server`\
 A folder you're [supposed to create](build.md#running-a-node), before starting your server: cd to project root; mkdir server; cd server; finn server start (or run) and it will create a subfolder .finn
  - `.finn`
    - `chain` - a database with the blockchain blocks and related information
    - `peers` - a database with the list of finn peers you're connected to
    - `txhashset` - contains folders kernel, rangeproof and output that each have a pmmr_dat.bin
- `src`\
  Code for the `finn` binary.
- `store`\
  Data store - finn uses near-zero-cost Rust wrapper around LMDB, key-value embedded data store.
- `target`\
  Where the finn binary ends up, after the compile and build process finishes.
...
- `wallet`\
  ...
  - `wallet_data` - a database storing your "outputs", that once confirmed and matured, can be spent with the [`finn wallet send`](wallet/usage.md) command. (locally created, *not* in git)
