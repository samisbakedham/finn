...
the algorithms and processes currently involved in finn's Proof-of-Work system. We'll start
with a general overview of cycles in a graph and the Cuckoo Cycle algorithm which forms the
basis of finn's proof-of-work. We'll then move on to finn-specific details, which will outline
the other systems that combine with Cuckoo Cycle to form the entirety of mining in finn.

Please note that finn is currently under active development, and any and all of this is subject to
(and will) change before a general release.

## Graphs and Cuckoo Cycle

finn's basic Proof-of-Work algorithm is called Cuckoo Cycle, which is specifically designed
...
technical aspects of finn's proof-of-work.
...
Now, (hopefully) armed with a basic understanding of what the Cuckoo Cycle algorithm is intended to do, as well as the parameters that affect how difficult it is to find a solution, we move on to the other portions of finn's POW system.

## Mining in finn

The Cuckoo Cycle outlined above forms the basis of finn's mining process, however finn uses Cuckoo Cycle in tandem with several other systems to create a Proof-of-Work.
...
Contrary to some existing concerns about Cuckoo Cycle's poolability, the POW implementation in finn as described above
is perfectly suited to a mining pool. While it may be difficult to prove efforts to solve a single graph in isolation,
the combination of factors within finn's proof-of-work combine to enforce a notion called 'progress-freeness', which
...
Following from this, finn's progress-freeness is due to the fact that a solution to a Cuckoo with finn's default parameters
