...
Bitcoin does something very similar, requiring 100 confirmations (Bitcoin blocks are every 10 minutes, finn blocks are every 60 seconds) before mining rewards can be spent.
...
A finn transaction consists of the following -
...
finn does not permit duplicate commitments to exist in the Output set at the same time.
...
This is not necessarily recommended but finn must handle this situation in a way that does not break consensus across the network.
...
finn does not allow duplicate commitments to exist in the Output set simultaneously.
