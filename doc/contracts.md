...
This document describes smart contracts that can be setup using finn even
though the finn chain does not support scripting. All these contracts rely
...
document or the finn development team. Most of the credit should be attributed
...
This section is meant as a reminder of some crucial features of the finn chain.
...
built for a finn transaction by taking the above private key `x` to be the sum
...
public key `r*G`, and allows to verify non-inflation for all finn transactions
(and signs the fees).
...
receiving party. As finn outputs are completely obscured by Pedersen
...
Alice wants to pay Bob in finns. She starts the transaction building process:
...
Alice has finns and Bob has bitcoin. They would like to swap. We assume Bob
...
ready to send her finns to Bob if he reveals `x`.

First, Alice sends her finns to a multiparty timelock contract with a refund
...
1. As soon as Bob broadcasts the final transaction to get his new finns, Alice
