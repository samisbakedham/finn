# finn Stratum RPC Protocol

*Read this in other languages: [Korean](translations/stratum_KR.md), [简体中文](translations/stratum_ZH-CN.md).*

This document describes the current Stratum RPC protocol implemented in finn.
...
A message initiated by the miner.
Miner can log in on a finn Stratum server with a login, password and agent (usually statically set by the miner program).
...
finn Stratum protocol implementation contains the following error message:
...
The current reference implementation is available at [mimblewimble/finn-miner](https://github.com/mimblewimble/finn-miner/blob/master/src/bin/client.rs).
