...
use crate::extkey_bip32::{BIP32finnHasher, ExtendedPrivKey, ExtendedPubKey};
use crate::types::{
...
impl ExtKeychain {
	...
	pub fn hasher(&self) -> BIP32finnHasher {
		self.hasher.clone()
	}
}
...
impl Keychain for ExtKeychain {
	fn from_seed(seed: &[u8], is_floo: bool) -> Result<ExtKeychain, Error> {
		let mut h = BIP32finnHasher::new(is_floo);
	...
	fn from_mnemonic(word_list: &str, extension_word: &str, is_floo: bool) -> Result<Self, Error> {
		let secp = secp::Secp256k1::with_caps(secp::ContextFlag::Commit);
		let h = BIP32finnHasher::new(is_floo);
