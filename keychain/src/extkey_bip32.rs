...
//! Modified from above to integrate into finn and allow for different
...
/// finn uses blake2 everywhere but the spec calls for SHA512/Ripemd160, so allow
...
#[derive(Clone, Debug)]
pub struct BIP32finnHasher {
	is_floo: bool,
	hmac_sha512: Hmac<Sha512>,
}

impl BIP32finnHasher {
	/// New empty hasher
	pub fn new(is_floo: bool) -> BIP32finnHasher {
		BIP32finnHasher {
			is_floo: is_floo,
...
impl BIP32Hasher for BIP32finnHasher {
	fn network_priv(&self) -> [u8; 4] {
		if self.is_floo {
			[0x03, 0x27, 0x3A, 0x10]
		} else {
...
impl ExtendedPrivKey {
	...
	pub fn from_mnemonic(
		...
		let mut hasher = BIP32finnHasher::new(is_floo);
